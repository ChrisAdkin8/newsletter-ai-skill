...
> 💡 When invoked from a local Claude Code session, this step uses your **Claude Code subscription quota** — no Anthropic API tokens are billed. See [`CLAUDE.md` → Publishing a new issue](../CLAUDE.md#publishing-a-new-issue), and [`docs/customising.md` → Scheduled publishing (launchd)](customising.md#scheduled-publishing-launchd) to run it every week without you.
