...
`/newsletter-ai` states rules it doesn't enforce, and the published issues break them. Two of the 25 items in `site/content/posts/2026-09-18.md` cite a rewrite of a primary the rewrite itself links to, which hard rule 7 forbids (`.claude/skills/newsletter-ai/SKILL.md:77`). Three citations across the two September issues point at rolling index pages, which hard rule 4 forbids (`.claude/skills/newsletter-ai/SKILL.md:74`). Every issue reprints a footer naming eight source families it never used. And the 18 Sep run published an issue but wrote no triage file, which the wrapper recorded as normal.
...
- **Two severities, not one.** A `never` rule holds the issue for the outlets `.claude/skills/newsletter-ai/sources.md:11-14` bans outright. A `secondary` rule for habitual rewrite outlets prints a warning that does not hold. The note's own Risks warn that false holds mute a gate, and `CLAUDE.md:22` forbids loosening a rule to get an issue out — so a taste judgement must not be able to block a push.
...
- **The monoculture rules stay prose.** A per-publisher cap and a ban on reused pivot phrases go into `.claude/skills/newsletter-ai/SKILL.md`, and the checker learns nothing about sections. The note's Counter-evidence argues these faults are invisible to a regex gate; teaching the checker sections is a Non-goal.
...
Read at `998811a` on 2026-09-18, working tree clean. The research note was verified the same day, and every claim borrowed below is CONFIRMED in its Verification table unless marked otherwise.
...
`wire` matches a host-list constant (`scripts/check_issue.py:32-38`) against every link (`scripts/check_issue.py:257-259`), while `homepage` and `label` run only over `[Source: …]` links (`scripts/check_issue.py:261-273`). The 18 tests in `tests/test_check_issue.py` drive the checker through `subprocess` (`tests/test_check_issue.py:5`) against the two May issues in `tests/fixtures` and an inline clean post, so changing `check()`'s return shape breaks no existing test.

**Two precedents this follows.** The banned-wire list lives in code and `.claude/skills/newsletter-ai/sources.md:16` points at it rather than repeating it ("**Press-release wires**: a hard rule in SKILL.md Step 2"). The new host lists do the same. And commit `0a35445` added the `future` rule (`scripts/check_issue.py:210-217`) this same week, so adding a rule to this checker is a known-size change.
...
The rolling pages are `site/content/posts/2026-09-13.md:89` (`code.claude.com/docs/en/changelog`), `site/content/posts/2026-09-13.md:257` (`epoch.ai/data/ai-data-centers`) and `site/content/posts/2026-09-18.md:281` ([`artificialanalysis.ai/changelog`](https://artificialanalysis.ai/changelog), where the cited v1.1 entry is fourth down although the permalink `/articles/artificial-analysis-capability-indices-v1-1` exists). All three sit inside `[Source: …]`, as does the fourth found while designing W2.
...
- Any link-resolution or liveness rule. The note's Counter-evidence records six hosts in the latest issue that answer a `HEAD` request with 403 or 405 while serving normally, and `.claude/skills/newsletter-ai/sources.md:22-24` already lists 14 domains that block automated fetches.
...
`check()` returns `(findings, warnings)`, both lists of `(line, rule, message)`. `main()` prints holds as it does today, then warnings as `path:line: warning: rule: message`, and returns 1 only if `findings` is non-empty. The `FINDING` regex at `tests/test_check_issue.py:14` captures `warning` as the rule on those lines, which is why the severity comes first: a warning can never be mistaken for a hold by a test, or by a reader of the `.check` file.
...
- `ROLLING`: a path whose last segment is `changelog`, `trending`, `releases`, `blog` or `docs`, or whose first segment is `data`.

`never` and `rolling` run over every link, as `wire` does, because hard rule 4 covers Quick Links too. `secondary` runs over `[Source: …]` links only, as `label` does, because the label is what claims the publisher.
...
**Done when**: over `site/content/posts/2026-09-13.md` and `site/content/posts/2026-09-18.md` the checker reports exactly three `rolling` findings — at `site/content/posts/2026-09-13.md:89`, `site/content/posts/2026-09-13.md:257` and `site/content/posts/2026-09-18.md:281` — and no other new rule fires. Over `site/content/posts/2026-05-14.md` it also reports `site/content/posts/2026-05-14.md:107`, `github.com/vllm-project/vllm/releases`, a releases index and so a true positive; the commit message says so, since that post is a test fixture and future readers will meet it.
...
**Done when**: over `site/content/posts/2026-09-18.md` the checker exits 0 for `secondary` alone and prints exactly two `warning: secondary:` lines, at `site/content/posts/2026-09-18.md:131` (Techzine) and `site/content/posts/2026-09-18.md:140` (The Register) — the two rule-7 breaches the research found, and nothing else. `make check` passes.
...
- Renumbering the rules changes the references to them elsewhere in the file: `.claude/skills/newsletter-ai/SKILL.md:65` ("up to the cap in the run rules"), `:87` ("Step 2, rule 5") and `:115` ("Step 2's hard rules") all need re-reading, and `.claude/skills/newsletter-ai/SKILL.md:69` describes what the checker can and can't see.

**Done when**: no numbered Step heading follows the "Output format" heading in `.claude/skills/newsletter-ai/SKILL.md`; rule 1 is the primary-or-specialist rule; `grep -n 'per publisher' .claude/skills/newsletter-ai/SKILL.md` hits in Step 2 and `grep -n 'hard rules' .claude/skills/newsletter-ai/SKILL.md` hits in Step 5; no reference to a rule by number points at the wrong rule; the description line (`.claude/skills/newsletter-ai/SKILL.md:3`) still says 14 categories; `make check` passes.

### W5: A missing triage file holds the issue

- `scripts/weekly.sh`: at Step 8, after the `git status` check (`scripts/weekly.sh:177-178`) and beside the checker call, require `$REPO/.newsletter/triage.md` to exist and `fail "held: no triage file; the model skipped Step 5"` if it doesn't. `.newsletter/` is in `.gitignore:1`, so the file's presence doesn't disturb `scripts/weekly.sh:177`'s expectation of exactly one untracked path. Leave `move_triage`'s own `no triage file` branch (`scripts/weekly.sh:71-74`) in place: it still covers the unpushed-issue path (`scripts/weekly.sh:140`), where the model didn't run at all.
- `tests/weekly_test.sh`: a fake `claude` that writes the post but no triage file leaves the tree dirty, exits non-zero, and notifies `held: no triage file`.
...
- `.claude/skills/newsletter-ai/template.md`: replace the footer (`.claude/skills/newsletter-ai/template.md:276`) with `*Curated by Claude Code*`, dropping the eight source families the issue doesn't use.
...
| Item | Estimate | Why |
|---|---|---|
| W1 | 30 min | One constant, a three-line rule modelled on `wire`, four tests, one prose edit |
| W2 | 1 h | The segment logic needs care against the 115 links in `site/content/posts/`; six tests |
| W3 | 1.5 h | `check()`'s return shape, `main()`'s output, `scripts/weekly.sh` and two test files |
| W4 | 1 h | 168 lines of prose to reorder without breaking the rule-number references |
| W5 | 30 min | Three lines plus an offline test case |
| W6 | 10 min | One line |

About half a day. W1–W3 are the note's highest-value change and land independently of W4–W6.
...
- **W3 changes the checker's output contract.** Anything parsing `$WEEK.check` meets a new line shape; `grep -rn 'WEEK.check' scripts tests` finds everything that does.
- **W4 edits the prompt that produces the issue.** A reorder can change the output in ways no test covers. The first issue after it wants reading before it is trusted, which is what `docs/customising.md` → "When an issue is held" already describes.

## Open questions

- Should the `secondary` warning eventually hold, once a few issues show how often it fires? The user chose warn-only for now; the count in the notification (W3) is the evidence that would settle it.
