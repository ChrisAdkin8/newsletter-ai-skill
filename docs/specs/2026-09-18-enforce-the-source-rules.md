...
1. ~~**Is `Write` available to the model under the wrapper's allowlist?**~~ **Answered, before the spike ran.** The 2026-W37 run wrote `.newsletter/triage.md` under exactly these flags: `~/Library/Logs/newsletter/launchd.log` notified "Published 2026-W37 … triage: 5 kept", and the resulting note is the 13 Sep triage file this spec cites in Background. `permission_denials` is empty in all three recorded runs. Note also that the skill's own `allowed-tools` (`.claude/skills/newsletter-ai/SKILL.md:6`) lists neither `Write` nor `Edit`, and has never stopped a post being written — so it isn't the gate either. W5 can therefore land in any order, and the `launchctl kickstart` precaution below is unnecessary.
...
3. ~~**Does `code.claude.com/docs/en/changelog` expose a per-version permalink or anchor?**~~ **Answered on 2026-09-18, during W2.** It doesn't: the page is generated from `CHANGELOG.md` and its `<Update>` elements carry no `id`, so there is no per-version anchor. But `github.com/anthropics/claude-code` publishes a page per release, and `/releases/tag/vX.Y.Z` passes `rolling`, so both catalogues now point there instead. The same resolution covers llama.cpp (`/releases/tag/bXXXX`).
...
- **W5 turns a soft failure into a weekly hold.** ~~If spike question 1 finds the write is denied, every run is held until the allowlist is fixed.~~ Retired: spike 1 is answered above — the W37 run wrote a triage file under these flags, so W5 holds only a run that genuinely skipped Step 5.
...
Reviewed on 2026-09-18 by spec-reviewer. Saved unchanged; not acted on.

| # | Kind | Where (exact text from the spec) | Finding | Affects | Evidence | What would settle it |
|---|---|---|---|---|---|---|
| 1 | COST | W3 Files: "`tests/test_check_issue.py`: a post citing `theregister.com` inside `[Source: …]` exits 0 and prints one `warning: secondary:` line" | `SECONDARY` fires on the existing test corpus, so W3 turns **8 of the 15 existing tests red**, and neither W3's Files nor its Done when budgets for rewriting them. The inline `CLEAN` post cites `theregister.com` inside `[Source:]`, so every CLEAN-based test that compares the whole finding list gains a `("warning", 15)` entry; both fixtures add more. The Background's reassurance is about the return shape only, but a cold reader will read it as "existing tests are safe". W3's own "`make check` passes" cannot hold as scoped. | correctness (fails W3's Done when "`make check` passes") | `tests/test_check_issue.py:31` (CLEAN's `[Source: [The Register](https://www.theregister.com/...)]`, = post line 15); exact-equality assertions at `tests/test_check_issue.py:131, 162, 164, 176, 187, 200, 208, 216`; `tests/fixtures/2026-05-15.md:38, 47, 56, 302` (theregister ×2, sherwood, dev.to — all in `[Source:]`) and `tests/fixtures/2026-05-14.md:38` (dev.to in `[Source:]`), which break the exact `expect(...)` blocks at `tests/test_check_issue.py:76-86` and `93-101`; `tests/test_check_issue.py:14` `FINDING` maps a warning line to rule `warning`; spec line 38 "so changing `check()`'s return shape breaks no existing test" | Add to W3's Files: update the 8 assertions (or switch `run()` to return severity+rule and split warnings out of `found`), and say in the Done when which of the 15 change |
| 2 | COST | W2 Done when: "Over `site/content/posts/2026-05-14.md` it also reports `site/content/posts/2026-05-14.md:107` … the commit message says so, since that post is a test fixture" | The spec spots that 05-14 is a fixture but only asks for a note in the commit message; the fixture is what `test_2026_05_14_on_its_own` asserts exactly, so W2 alone turns that test red. W2's Files list `tests/test_check_issue.py` for new cases only. W1's Done when ("`python3 -m unittest discover tests` passes") is then unreachable once W2 lands. | correctness (fails W1/W2's Done when "`python3 -m unittest discover tests` passes with the new tests") | `tests/fixtures/2026-05-14.md:107` = `https://github.com/vllm-project/vllm/releases`; `tests/test_check_issue.py:93-101` asserts `expect(label=[38], repeat=[116,248], homepage=[116,239,248,311], stale=[77,146,227,320])` with no `rolling` | W2's Files say "add `('rolling', 107)` to `test_2026_05_14_on_its_own`'s `expect(...)`" |
| 3 | COST | W3 Files: "`scripts/weekly.sh`: after Step 8's checker call (`scripts/weekly.sh:179-180`) succeeds, count the `: warning: ` lines in `$LOG/$WEEK.check` and carry the count into the Step 12 notification (`scripts/weekly.sh:194`)" | Step 8 lives in the `else` branch; the push-retry branch jumps straight from `git push` to Steps 10-12. A count set only at Step 8 is unset when Step 12 runs on a retry, and `set -uo pipefail` aborts the script at `notify` — after `last-ok` and `move_triage`, with no notification and a non-zero exit. That is exactly the recovery path `weekly_test.sh` asserts on, so the offline test goes red too. (Computing it at Step 12 instead needs a guard: `grep -c` on a missing `.check` file yields an empty string and exit 2.) | correctness (breaks the documented failed-push retry; fails W3's "`make check` passes") | `scripts/weekly.sh:15` `set -uo pipefail`; branch split at `scripts/weekly.sh:135-139` vs `140-185`; `scripts/weekly.sh:188-194` runs for both; `tests/weekly_test.sh:158-163` `check "retry exits 0" eq "$RC" 0` | Initialise the count (e.g. `WARN_NOTE=`) before `if [ -n "$retry" ]`, and say so in W3's Files |
| 4 | COST | W5 Files (only `docs/customising.md` "When an issue is held"); W3 Files (only `docs/customising.md:279` and `docs/customising.md:314`) | Four prose statements of the behaviour these items change are in no Files list, so they go stale: two state the hold conditions (W5 adds a third), two state what the notification carries (W3 adds the warning count). W4 has the same gap for the per-publisher cap, which sits beside the documented "up to 4 per category". | requirement: "It holds the issue unless the only change in the tree is the new post and `scripts/check_issue.py` passes it" | `docs/how-it-works.md:198` and `docs/customising.md:274` (hold conditions); `docs/how-it-works.md:200` ("sends a notification with the run's cost") and `docs/customising.md:275`; `docs/how-it-works.md:102` ("up to 4 high-quality items per category") | Add `docs/how-it-works.md:198`/`:200` and `docs/customising.md:274`/`:275` to W5's and W3's Files, and `docs/how-it-works.md:102` to W4's |
| 5 | ASSUMPTION | W2 Files: "four catalogue entries across the two parallel catalogues name a URL `rolling` would hold" … "Read the two new tables through once and mark any other row whose URL `rolling` would hold" | The count is wrong by a wide margin and the enumeration contradicts itself (it names three locations per copy, i.e. six, not four). At `c9ae1fd`, **63 lines in each catalogue** carry a URL whose path `rolling` would hold — nearly every vendor blog index, plus `huggingface.co/papers/trending`, `ossinsight.io/trending` and four GitHub `/releases` pages. So the closing instruction is a ~126-row edit, not a read-through, and marking them all risks telling the model not to *use* sources the search strategies point it at. The Risks paragraph's "contradicts two entries in the catalogue" rests on the same undercount. | neither: no Done when covers the catalogue, so the checker, the tests and the two Sept posts still behave as specified — but W2's 1.25 h estimate and the rollback reasoning are built on a count that is off by ~20× | `git show HEAD:.claude/skills/newsletter-ai/sources.md \| grep -cE 'https?://[^ )`]*/(changelog\|trending\|releases\|blog\|docs)/?([ )`,]\|$)'` → 63; same for `docs/sources.md` → 63; e.g. `.claude/skills/newsletter-ai/sources.md:608` (`cncf.io/blog/`), `:655` (`ossinsight.io/trending`), `:622`/`:625`/`:627` (GitHub `/releases`) | Decide the rule for the catalogue (mark only rows cited as *the* source of a release, leave "where to look" rows alone), state that rule in W2, and re-estimate |
| 6 | ASSUMPTION | Spike 1: "The run logs record no permission denials, which argues it is available, but that is inference *(assumption)*. Cheapest experiment: a headless `claude -p` in a scratch copy…" | Already answered by evidence the spec itself leans on: a wrapper run of the same flags wrote `.newsletter/triage.md` — the 2026-W37 run notified "triage: 5 kept", and the spec cites the resulting 13 Sep triage note in Background. The spike also never mentions the second gate a cold reader will trip over, `allowed-tools: WebSearch, WebFetch, Read` in the skill's own frontmatter, which lists neither `Write` nor `Edit` yet demonstrably does not stop the post being written. | neither: retires spike 1, the risk "If spike question 1 finds the write is denied, every run is held", and the ordering constraint "Land W5 last" — no work item changes | research note sources 3 (`launchd.log`: "Published 2026-W37 … triage: 5 kept") and 1 (`permission_denials` empty in all three runs); spec line 48 (the 13 Sep triage note exists); `.claude/skills/newsletter-ai/SKILL.md:6` `allowed-tools: WebSearch, WebFetch, Read`; four posts exist under those flags | Replace spike 1 with the W37 log line as evidence, and note why the skill's `allowed-tools` isn't the gate |
| 7 | ASSUMPTION | Goal: "Every issue reprints a fixed footer naming 15 source families, eight of which the 18 Sep issue never used" (also lines 48 and 152) | Undercount: Covington is also named in the footer and is not among that issue's 28 hosts, so it is nine of 15, not eight. | neither: W6 deletes the whole list, so the motivation stands either way | `.claude/skills/newsletter-ai/template.md:276` names "… IAPP, Covington, Ada Lovelace Institute …"; no `covington` host in `site/content/posts/2026-09-18.md` | Recount against the 28 hosts, or drop the number |
| 8 | COLD-READ | W1 Done when: "the checker over a post containing the 13 Sep triage note's `fool.com` URL prints a `never` line and exits 1" | The URL is nowhere in the spec or the repo — it lives in `~/notes/inbox/2026-09-13-newsletter-triage.md`, and W1's test bullet elides it (`www.fool.com/investing/2026/09/09/…`). An implementer has to leave the repo to run its own Done when. | neither: any `fool.com` URL satisfies `never`, so the check is reproducible once the reader realises the exact URL doesn't matter | spec lines 48, 96, 102; research note source 10 ("`~/notes/inbox/2026-09-13-newsletter-triage.md`: first item links `fool.com/investing/2026/09/09/...`") | Paste the URL into W1, or reword the Done when to "any `fool.com` URL" |

Counts: 8 findings — 3 correctness, 1 requirement, 4 neither
Neither: 5, 6, 7, 8
Cold read: no — W1's Done when, which needs the 13 Sep triage note's `fool.com` URL from `~/notes/inbox`
New spike questions: none (row 6 retires spike 1; spike 3 is still open at `c9ae1fd`)
